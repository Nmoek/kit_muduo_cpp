/**
 * @file test_custom_pattern_v2.cpp
 * @brief 自定义TCP协议格式V2测试
 * @author Kewin Li
 * @version 1.0
...
// 由 test_custom_project_server.h 中的 pattern_json_str1 转换而来：
// least_byte_len -> header_bytes，special_fields -> fields + role。
// 策略 "body_length"的测试数据
const std::string kPatternJsonStr1V2 = R"({
  "version": 2,
  "header_bytes": 26,
  "default_order": "big",
...
const std::string kPatternJsonStr2_1V2 = R"({
  "version": 2,
  "header_bytes": 26,
...
const std::string kPatternJsonStr2_2V2 = R"({
  "version": 2,
  "header_bytes": 4,
  "default_order": "big",
  "length_policy": "total_length",
...
// 旧 least_byte_len 只覆盖特殊字段；V2 的 header_bytes 需要覆盖完整可序列化头。
// 策略 "no_length" 的测试数据。
const std::string kPatternJsonStr3V2 = R"({
  "version": 2,
  "header_bytes": 24,
  "default_order": "raw",
...
{
    auto spec_opt = CustomTcpPatternSpec::FromJson(nljson::parse(kPatternJsonStr1V2));
    if(!spec_opt.has_value())
    {
        ADD_FAILURE() << "failed to parse kPatternJsonStr1V2";
        throw std::invalid_argument("kPatternJsonStr1V2 invalid!");
    }
...
{
    auto spec_opt = CustomTcpPatternSpec::FromJson(nljson::parse(kPatternJsonStr2_1V2));
    if(!spec_opt.has_value())
    {
        ADD_FAILURE() << "failed to parse kPatternJsonStr2_1V2";
        throw std::invalid_argument("kPatternJsonStr2_1V2 invalid!");
    }
...
{
    auto spec_opt = CustomTcpPatternSpec::FromJson(nljson::parse(kPatternJsonStr2_2V2));
    if(!spec_opt.has_value())
    {
        ADD_FAILURE() << "failed to parse kPatternJsonStr2_2V2";
        throw std::invalid_argument("kPatternJsonStr2_2V2 invalid!");
    }
    return TotalLengthPattern(spec_opt.value());
}
...
{
    auto spec_opt = CustomTcpPatternSpec::FromJson(nljson::parse(kPatternJsonStr3V2));
    if(!spec_opt.has_value())
    {
        ADD_FAILURE() << "failed to parse kPatternJsonStr3V2";
        throw std::invalid_argument("kPatternJsonStr3V2 invalid!");
    }
...
void ExpectParsedFieldHex(const CustomTcpPatternV2::ParseHeaderResult& result,
                          size_t byte_pos,
...
CustomTcpItemCfg MakeReqCfg1V2()
{
    CustomTcpItemCfg cfg;
    cfg.function_code_hex = "H0100";
...
CustomTcpItemCfg MakeRespCfg1V2()
{
    CustomTcpItemCfg cfg;
    cfg.function_code_hex = "H1080";

    // 由旧 resp_cfg1 转换：V2 中 common 字段值必须严格等于字段 byte_len。
    cfg.field_values_by_byte_pos.emplace(
...
CustomTcpItemCfg MakeReqCfg2_1V2()
{
    CustomTcpItemCfg cfg;
    cfg.function_code_hex = "H0100";
...
CustomTcpItemCfg MakeRespCfg2_1V2()
{
    CustomTcpItemCfg cfg;
    cfg.function_code_hex = "H0180";
...
CustomTcpItemCfg MakeReqCfg2_2V2()
{
    CustomTcpItemCfg cfg;
    cfg.function_code_hex = "H32";
    return cfg;
}

CustomTcpItemCfg MakeRespCfg2_2V2()
{
    CustomTcpItemCfg cfg;
    cfg.function_code_hex = "H42";
    return cfg;
}

std::vector<uint8_t> MakeReqBody2_2V2()
{
...
std::vector<uint8_t> MakeRespBody2_2V2()
{
    // 由旧 MakeResp2_2 转换：
...
CustomTcpItemCfg MakeReqCfg3V2()
{
    CustomTcpItemCfg cfg;
    cfg.function_code_hex = "H3031";
...
CustomTcpItemCfg MakeRespCfg3V2()
{
    CustomTcpItemCfg cfg = MakeReqCfg3V2();
    cfg.function_code_hex = "H3131";
    return cfg;
}
...
/*
测试思路：
  旧 pattern_json_str1 是“头中有 body 长度”的格式，V2 解析时应按 byte_pos
...
举例：
  req_cfg1 的功能码 H0100 应被解析为 result.function_code_hex，
  body 长度字段应按 V2 自动回写为 H0000000F。
*/
TEST(TestCustomTcpPatternV2, ParseHeaderWithConvertedPatternJsonStr1AndReqCfg1)
{
    auto pattern = MakePatternJsonStr1Pattern();
    const std::vector<uint8_t> req_body = BytesOf(R"({"key1":"val1"})");

    auto serialized = pattern.serialize(MakeReqCfg1V2(), req_body);
    ASSERT_TRUE(serialized.has_value());

    auto result = pattern.parseHeader(*serialized);
    ASSERT_TRUE(result.ok()) << "status=" << result.status;
    EXPECT_EQ(result.function_code_hex, "H0100");
...
/*
测试思路：
  resp_cfg1 + resp_body1 用于验证 V2 serialize 的完整输出：固定魔数来自
...
*/
TEST(TestCustomTcpPatternV2, SerializeRespCfg1AndRespBody1PatchesBodyLength)
{
    auto pattern = MakePatternJsonStr1Pattern();
    const std::vector<uint8_t> resp_body = BytesOf(kRespBody1);
    ASSERT_EQ(resp_body.size(), 21u);

    auto serialized = pattern.serialize(MakeRespCfg1V2(), resp_body);
    ASSERT_TRUE(serialized.has_value());
    ...
    auto result = pattern.parseHeader(*serialized);
    ASSERT_TRUE(result.ok()) << "status=" << result.status;
    EXPECT_EQ(result.function_code_hex, "H1080");
...
测试思路：
  ...
  所有数值字段采用 little-endian，因此 V2 的 default_order 也必须改成 little。
...
*/
TEST(TestCustomTcpPatternV2, ParseHeaderWithConvertedPatternJsonStr2_1AndReqCfg2_1)
{
    ...
    auto serialized = pattern.serialize(MakeReqCfg2_1V2(), req_body);
    ASSERT_TRUE(serialized.has_value());
...
*/
TEST(TestCustomTcpPatternV2, SerializeRespCfg2_1AndRespBody2_1PatchesTotalLength)
{
    auto pattern = MakePatternJsonStr2_1Pattern();
    const std::vector<uint8_t> resp_body = BytesOf(kRespBody2_1);
    ASSERT_EQ(resp_body.size(), 21u);

    auto serialized = pattern.serialize(MakeRespCfg2_1V2(), resp_body);
    ASSERT_TRUE(serialized.has_value());
    ...
    auto result = pattern.parseHeader(*serialized);
    ASSERT_TRUE(result.ok()) << "status=" << result.status;
    EXPECT_EQ(result.function_code_hex, "H0180");
    EXPECT_EQ(result.remain_body_bytes, resp_body.size());
...
测试思路：
  ...
  报文长度表示整包总长度，和 V2 total_length = header_bytes + body.size() 一致。
  文档响应还包含“判图结果”字段，因此 V2 测试要把这个字段补齐到 body。
...
举例：
  req_cfg2_2 没有 common_fields，只提供功能码 H32；serialize 应自动写总长度 H000E，
  parseHeader 应返回 remain_body_bytes=10。响应补齐判图结果后，body 为 18 字节，
  序列化后的整包总长度为 4 + 18 = 22，即 H0016。
*/
TEST(TestCustomTcpPatternV2, SerializeAndParseConvertedPatternJsonStr2_2TotalLength)
{
    auto pattern = MakePatternJsonStr2_2Pattern();

    const std::vector<uint8_t> req_body = MakeReqBody2_2V2();
    ASSERT_EQ(req_body.size(), 10u);

    auto req_serialized = pattern.serialize(MakeReqCfg2_2V2(), req_body);
    ...
    EXPECT_EQ(req_result.function_code_hex, "H32");
    ...
    const std::vector<uint8_t> resp_body = MakeRespBody2_2V2();
    ASSERT_EQ(resp_body.size(), 18u);

    auto resp_serialized = pattern.serialize(MakeRespCfg2_2V2(), resp_body);
    ...
    EXPECT_EQ(resp_result.function_code_hex, "H42");
...
/*
测试思路：
  旧 pattern_json_str3 没有长度字段。V2 中需要把旧 req_cfg3/resp_cfg3 的普通头字段
...
*/
TEST(TestCustomTcpPatternV2, SerializeAndParseConvertedPatternJsonStr3NoLength)
{
    auto pattern = MakePatternJsonStr3Pattern();

    auto req_serialized = pattern.serialize(MakeReqCfg3V2(), {});
    ASSERT_TRUE(req_serialized.has_value());
    ...
    auto req_result = pattern.parseHeader(*req_serialized);
    ASSERT_TRUE(req_result.ok()) << "status=" << req_result.status;
    EXPECT_EQ(req_result.function_code_hex, "H3031");
    ...
    auto resp_serialized = pattern.serialize(MakeRespCfg3V2(), {});
    ASSERT_TRUE(resp_serialized.has_value());
    ...
    auto resp_result = pattern.parseHeader(*resp_serialized);
    ASSERT_TRUE(resp_result.ok()) << "status=" << resp_result.status;
    EXPECT_EQ(resp_result.function_code_hex, "H3131");
...
举例：
  pattern_json_str1V2 声明 26 字节头，只给 25 字节时 fields_value 应为空。
*/
TEST(TestCustomTcpPatternV2, ParseHeaderRejectsDataShorterThanHeader)
{
    auto pattern = MakePatternJsonStr1Pattern();
    std::vector<uint8_t> short_data(pattern.spec().header_bytes - 1, 0x00);

    auto result = pattern.parseHeader(short_data);
    EXPECT_EQ(result.status, CustomTcpPatternV2::ParseHeaderResult::kNonMinLength);
    EXPECT_FALSE(result.ok());
    EXPECT_TRUE(result.fields_value.empty());
    EXPECT_TRUE(result.function_code_hex.empty());
...
*/
TEST(TestCustomTcpPatternV2, ParseHeaderRejectsStartMagicMismatch)
{
    auto pattern = MakePatternJsonStr1Pattern();
    auto serialized = pattern.serialize(MakeReqCfg1V2(), BytesOf(R"({"key1":"val1"})"));
    ASSERT_TRUE(serialized.has_value());
    (*serialized)[0] = 0x24;

    auto result = pattern.parseHeader(*serialized);
    EXPECT_EQ(result.status, CustomTcpPatternV2::ParseHeaderResult::kInvalidDataError);
    EXPECT_FALSE(result.ok());
    EXPECT_TRUE(result.function_code_hex.empty());
}
...
/*
测试思路：
  common 字段在 V2 serialize 阶段允许只配置部分字段。PatternSpec 中声明了
...
*/
TEST(TestCustomTcpPatternV2, SerializeAllowsPartialCommonFieldOverrides)
{
    auto pattern = MakePatternJsonStr1Pattern();
    CustomTcpItemCfg cfg = MakeReqCfg1V2();
...
*/
TEST(TestCustomTcpPatternV2, SerializeRejectsCommonFieldLengthMismatch)
{
    auto pattern = MakePatternJsonStr1Pattern();
    CustomTcpItemCfg cfg = MakeReqCfg1V2();
...
/*
测试思路：
  功能码字段同样必须满足 byte_len。V2 不应接受短功能码或不带 H 前缀的伪十六进制值，
  否则运行期按功能码索引协议项会出现歧义。
...
举例：
  pattern_json_str1V2 的功能码是 2 字节，H01 和 0100 都不能被写入。
*/
TEST(TestCustomTcpPatternV2, SerializeRejectsInvalidFunctionCodeHex)
{
    auto pattern = MakePatternJsonStr1Pattern();
    CustomTcpItemCfg cfg = MakeReqCfg1V2();

    cfg.function_code_hex = "H01";
    EXPECT_FALSE(pattern.serialize(cfg, BytesOf(R"({"key1":"val1"})")).has_value());

    cfg.function_code_hex = "0100";
    EXPECT_FALSE(pattern.serialize(cfg, BytesOf(R"({"key1":"val1"})")).has_value());
}
...
*/
TEST(TestCustomTcpPatternV2, SerializeAllowsEmptyBodyAndPatchesZeroLength)
{
    auto pattern = MakePatternJsonStr1Pattern();
    const std::vector<uint8_t> empty_body;

    auto serialized = pattern.serialize(MakeRespCfg1V2(), empty_body);
    ASSERT_TRUE(serialized.has_value());
...
举例：
  ...
  body 为 256 字节时不能序列化。
*/
TEST(TestCustomTcpPatternV2, SerializeChecksBodyLengthFieldOverflow)
{
