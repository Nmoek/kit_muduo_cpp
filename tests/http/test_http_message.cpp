...
2. 底层 Body 不应把缺失类型猜成 text/json。
3. 缺省应按 octet-stream 处理，保留原始字节语义。

示例：
  HTTP/1.1 200 OK + Content-Length:6 + no Content-Type
        |
        v
  body.contentType == kOctetStream
*/
...
3. Body::data 和 toString 都应保留完整 5 字节。

示例：
  bytes={'a','\0','b','\0','c'}
        |
        v
  body.size == 5, contentType == kOctetStream
*/
