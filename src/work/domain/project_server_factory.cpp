...
std::shared_ptr<ProjectServer> HttpProjectServerCreator::create(const kit_domain::Project &p) 
{
    ...
    try {
        ...
        PJSERVER_ERROR() << "Failed to create HttpProjectServer: " << e.what() << std::endl;
        return nullptr;
    }
}
...
std::shared_ptr<ProjectServer> HttpsProjectServerCreator::create(const kit_domain::Project &p) {
    ...
    // TODO: 实现HTTPS服务器创建逻辑
    PJSERVER_WARN() << "HttpsProjectServer not implemented yet, using HttpProjectServer" << std::endl;
    return nullptr;
}
...
std::shared_ptr<ProjectServer> TcpProjectServerCreator::create(const kit_domain::Project &p) 
{
    ...
    try {
        ...
        PJSERVER_ERROR() << "Failed to create CustomTcpProjectServer: " << std::endl << e.what() << std::endl;
        return nullptr;
    }
}
...
void ProjectServerFactoryManager::initializeDefaultCreators() {
    PJSERVER_DEBUG() << "Initializing default ProjectServer creators" << std::endl;
    ...
    PJSERVER_INFO() << "Default ProjectServer creators initialized successfully" << std::endl;
}

void ProjectServerFactoryManager::registerCreator(std::unique_ptr<ProjectServerCreator> creator) {

    if (!creator) {
        PJSERVER_ERROR() << "Attempted to register null creator" << std::endl;
        return;
    }
    ...
    if (result.second) {
        PJSERVER_DEBUG() << "Registered creator for protocol type: " << static_cast<int32_t>(protocol_type) << std::endl;
    } else {
        PJSERVER_WARN() << "Creator for protocol type " << static_cast<int32_t>(protocol_type) 
...
std::shared_ptr<ProjectServer> ProjectServerFactoryManager::createServer(const kit_domain::Project &p) 
{
    ...
    if (it == creators_.end()) {
        PJSERVER_ERROR() << "Unsupported protocol type: " << static_cast<int32_t>(p.m_protocolType) << std::endl;
        return nullptr;
    }
    
    PJSERVER_DEBUG() << "Creating ProjectServer with protocol type: " << static_cast<int32_t>(p.m_protocolType)
