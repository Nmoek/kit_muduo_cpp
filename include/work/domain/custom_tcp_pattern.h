...
class CustomTcpPatternBase : public CustomTcpPattern
{
public:
    CustomTcpPatternBase(const CustomTcpPatternSpec &spec)
        :pattern_spec_(spec)
    {

    }

    ~CustomTcpPatternBase() override = default;
    ...
    ParseHeaderResult parseHeader(const std::vector<uint8_t> &data) override
    {
        ...
        result.status = checkFunctionCode(result.fields_value, result.function_code);
        if(!result.ok())
        {
            return result;
        }
...
class BodyLengthPattern final: public CustomTcpPatternBase<BodyLengthPattern>
{
public:
...
class TotalLengthPattern final: public CustomTcpPatternBase<TotalLengthPattern>
{
public:
...
class NoLengthPattern final: public CustomTcpPatternBase<NoLengthPattern>
{
public:
...
public:
    ...
    static std::shared_ptr<CustomTcpPattern> Create(const nlohmann::json& json);
};

}
#endif //__KIT_CUSTOM_TCP_PATTERN_H__
